...
MATCHES = {
    ...
    '<strong>Python\'s <a href="https://docs.python.org/3/glossary.html">glossary</a> defines iterable as any object with special methods iter() or getitem(), and sequence as any object with getitem() and len(). It doesn\'t define the term <em>collection</em>.</strong>': '<strong>Python\'s glossary defines iterable as any object with special methods iter() or getitem(), and sequence as any object with getitem() and len(). It doesn\'t define the term <em>collection</em>.</strong>',
    '<strong>Using <a href="#abstractbaseclasses">ABC</a> Iterable with isinstance() or issubclass() only checks whether object/class has special method iter(), while ABC Collection checks for iter(), contains() and len().</strong>': '<strong>Using ABC Iterable with isinstance() or issubclass() only checks whether object/class has special method iter(), while ABC Collection checks for iter(), contains() and len() (p. 4).</strong>',
