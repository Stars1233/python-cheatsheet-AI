...
MATCHES = {
    ...
    '<strong>This text uses the term collection instead of <a href="#abstractbaseclasses">iterable</a>. For rationale see <a href="#iterableducktypes">duck types</a>.</strong>': '<strong>This text uses the term <em>collection</em> instead of <em>iterable</em>. For rationale see duck types (p. 4, 18).</strong>',
    ...
    '<strong>Install a WSGI server like <a href="https://flask.palletsprojects.com/en/latest/deploying/waitress/">Waitress</a> and a HTTP server such as <a href="https://flask.palletsprojects.com/en/latest/deploying/nginx/">Nginx</a> to get better security.</strong>': '<strong>Install a WSGI server like Waitress and a HTTP server such as Nginx to get better security.</strong>',
